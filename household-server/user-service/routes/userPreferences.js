...
router.get("/", verifyToken, async (req, res) => {
    try {
        ...
        logger.error(
            "Error fetching user preferences from PostgreSQL:",
            error.message,
        )
...
router.post("/", verifyToken, async (req, res) => {
    ...
    try {
        ...
        logger.error(
            "Error creating user preferences in MongoDB:",
            error.message,
        )
        res.status(500).json({ error: error.message })
    }
...
router.delete("/", verifyToken, async (req, res) => {
    try {
        const preferences = await UserPreference.findOneAndDelete({
            user_id: req.user_id,
        })
        ...
        logger.error("Error deleting user preferences:", error.message)
        res.status(500).json({ error: error.message })
    }
})
